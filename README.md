...
四个作品面向不同用户、问题和产品形态，成熟度也不相同。我会分别用源码、交互截图、测试、质量检查脚本和明确的局限说明它们当前做到了什么，而不是把所有原型包装成已经完成商业验证的产品。
...
这组作品覆盖了四种不同的 Vibe Coding 任务：

1. **做一个完整 AI Web 产品**：从移动端交互、异步任务到部署与成本复盘。
2. **把专业工作流封装成 Skill**：让教学、资料标准和状态管理可重复执行。
3. **用多阶段 Agent 生成内容**：拆开需求、文案、视觉决策和图像生成。
4. **用对话替代表单**：把用户的自然表达逐步转成结构化结果。
...
| 能力 | 仓库里的具体实现 | 可以检查的证据 |
| --- | --- | --- |
| 产品问题拆解 | 将“现有食材怎么吃”“AI PM 学习状态丢失”“电商图文流程割裂”“不会写简历”拆成四种产品流程 | 四个项目的问题定义、用户旅程和边界说明 |
| 用户交互设计 | 拍照—校正—生成—换菜—做菜；对话式简历采集；Gradio 海报流水线 | 圆头耄耋前端、两个 Notebook 和产品截图 |
| Agent / Skill 编排 | 菜单生成与审核 Skills、课程老师 Skill、海报四阶段流水线 | `圆头耄耋智能配菜Agent/backend/skills/`、`Skills作品/aipm-course-teacher/SKILL.md`、AIGC Notebook |
| 结构化输出约束 | Pydantic、JSON Schema、Zod、结构化简历快照、简报解析 | `圆头耄耋智能配菜Agent/backend/app/models.py`、`圆头耄耋智能配菜Agent/backend/app/contracts.py`、`圆头耄耋智能配菜Agent/frontend/lib/contracts.ts`、两个 Notebook |
| LLM 与代码分工 | LLM 负责视觉识别、菜单规划和语言生成；代码负责热量、预算、完成度、锁和版本 | `圆头耄耋智能配菜Agent/backend/app/calories.py`、`圆头耄耋智能配菜Agent/backend/app/channels.py`、课程质量脚本、Notebook 评分函数 |
...
这个仓库展示的是产品假设、实现能力和工程思考，不代表四个方向都已经完成用户研究、商业化或生产验证。
